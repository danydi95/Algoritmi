...
///metodo per estrarre l'elemento di distanza minima
...
template <class H,class T> int Dijkstra(Graph<H,T>* graph, H keysrc, T*& distances, int*& predecessors)
{
	...
	while(!q.empty())
	{
		...
		if(distances[tmp]==INT_MAX)
			break;
		for(int i=0;i<V;i++) //cercando i vicini
		{
...
//metodo per stampare le distanze del grafo
template <class H,class T> int DistancesTo(Graph<H,T>* graph, H key,H des ,T* distances, int* predecessors)
{
...
int main()
{
	...
	while(getline(input,line))
	{
		...
		if(Dijkstra(g,start_node,distances,predecessors))
		{
