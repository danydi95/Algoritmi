...
int IsAdded(int x, vector<int> A)
{
...
int main()
{
	...
	while(getline(input,line))
	{
		...
		int iterations=stoi(tokens[2]);
		int start_node=stoi(tokens[edges*3+3]);
		int end_node=stoi(tokens[edges*3+4]);
		g = new Graph<int,int>(nodes);
		int *a = new int[edges];
		int *b = new int[edges];
		int *c = new int[edges];
		vector<int> nodi;
		...
		{
			string value=tokens[3+i];
			if(value.at(0)=='(')
			{
				int d = stoi(value.erase(0,1));
			...
			{
				int d = stoi(value.erase(value.length()-1,value.length()));
		...
		{
			//cout << "Aggiungo nodo: " << nodi[i] << endl;
			g->addNode(nodi[i]);
		}
